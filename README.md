...
k3air 是一个使用 Go 编写在离线环境下安装高可用 k3s 集群的命令行工具，它不依赖任何外部服务网络，所有的组件都在本地运行。k3air 支持一个 yaml 格式的配置文件，用户可以在配置文件中定义 k3s 集群的节点，网络，启动参数等信息，所有 依赖都打包成离线文件，然后通过 k3air 命令来 k3s 集群。
